...
- Base code (files you will certainly edit):
	...
	- [`PlayMode.hpp`](PlayMode.hpp), [`PlayMode.cpp`](PlayMode.cpp) runs the story: layout, effects, input.
	- [`TextRenderer.hpp`](TextRenderer.hpp), [`TextRenderer.cpp`](TextRenderer.cpp) HarfBuzz shaping + FreeType glyph atlas + batched quad drawing.
	- [`Story.hpp`](Story.hpp), [`Story.cpp`](Story.cpp) loads `dist/story.bin`; [`story.xml`](story.xml) is the script and [`compile_story.py`](compile_story.py) compiles it.
...
- Useful code (files you should investigate, but probably won't change):
	...
	- shaders (you might also build on these):
		...
		- [`TextProgram.hpp`](TextProgram.hpp), [`TextProgram.cpp`](TextProgram.cpp) GLSL shader that draws glyph quads (vertex color, alpha from a single-channel atlas).
